...
- `fixtures/reference/`: Committed reference datasets (`inputs/`), golden outputs (`indicators/`), and backend-family fixtures (`bar_window/`, `exit_rules/`, `candle_engine/`, `decision_step/`).
...
- `make fixtures-backend` / `make fixtures-backend-check`: Export / verify backend families (`BACKEND_FAMILIES`, currently `bar_window`, `exit_rules`, `candle_engine`, and `decision_step`) that require the full `q_backend` locked environment. Required locally before merging changes to `tools/reference/families/bar_window.py`, `tools/reference/families/exit_rules.py`, `tools/reference/families/candle_engine.py`, `tools/reference/families/decision_step.py`, the matching fixture subtrees, or `BACKEND_REV`. Not run in CI: the sync pulls multi-GB wheels (torch / nvidia-*), while every `cargo test` already fail-closes on a pin edit without regenerated fixtures via `ProvenanceRev`.
